...
By default, this API will require authentication when called with an [field]#applicationId#. Authentication may be disabled per Application, see [field]#application.loginConfiguration.requireAuthentication# in the link:../apis/applications[Application API] or navigate to [breadcrumb]#Applications# icon:chevron-right[role=breadcrumb,type=fas] [breadcrumb]#Edit# icon:chevron-right[role=breadcrumb,type=fas] [breadcrumb]#Security# in the user interface.


Prior to version `1.5.0` this API did not accept an API key and never required authentication.

[.api-authentication]
link:authentication#no-authentication-required[icon:unlock-alt[role=green,type=far]] link:authentication#no-authentication-required[icon:lock[role=red,type=far]] Authenticate a User
[.endpoint]
.URI
--
...
The user's password or an Application specific link:../tutorials/application-authentication-tokens[Authentication Token].
...
[.api-authentication]
link:authentication#no-authentication-required[icon:unlock-alt[role=green,type=far]] Complete a Two Factor Login
[.endpoint]
.URI
--
...
on the link:users#change-a-users-password[Change Password] API. Since version `1.15.0`, the response will also contain a `changePasswordReason` field which can have one of the following values:
...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red,type=far]] Update the login instant for a given User and Application
[.endpoint]
.URI
--
...
[.api-authentication]
link:authentication#jwt-authentication[icon:id-badge[role=green,type=far]] Update the login instant for a given User and Application using a JWT
[.endpoint]
.URI
--
...
[.api-authentication]
link:authentication#no-authentication-required[icon:unlock-alt[role=green,type=far]] Remove the `access_token` and `refresh_token` cookies if they exist on the client.
[.endpoint]
.URI
--
...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red,type=far]] Searches for Login Records given search criteria
[.endpoint]
.URI
--
...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red,type=far]] Export the Login Records matching the criteria
[.endpoint]
.URI
--
